		...
		while (path[startIndex] != '\0')
		{
			...
			while (path[startIndex] != '\0')
			{
				...
				else
				{
					...
					{
						throw InvalidPathException(safeContextSubstr(path, startIndex, 20), MyString("Not allowed symbol in non-quoted block key. Symbol: ") + MyString(path[startIndex + 1]));
