...
JsonElementBase *JsonElementParser::parsePrimitive(MyString elementString)
{
	...
	{
		MyString val = elementString.substr(1, elementString.getLength() - 2);
		for (size_t i = 0; i < val.getLength(); i++)
		{
			if (val[i] == '"' && (i == 0 || val[i - 1] != '\\'))
			{
				throw InvalidJsonException(elementString, "Invalid Json String. Unescaped quote found in the string.");
			}
		}

		return new StringJsonElement(val);
	}
