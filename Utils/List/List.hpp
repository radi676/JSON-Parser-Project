...
template <typename T>
class List // vector implementation
{
protected:
    T *data;
    size_t count;
...
public:
    List();
    explicit List(size_t capacity); // not a convert constructor
    ...
    template <typename K>
    bool removeFirstWhere(bool (*predicate)(const T &check, const K &valueCheck), K valueCheck);
...
template <typename T>
List<T>::List(size_t capacity) // memory allocation only
{
    count = 0;
...
bool List<T>::removeAt(size_t index)
{
    ...
    if (count * RESIZE_FACTOR * RESIZE_FACTOR <= capacity) // prevent further resize if not enough space
    {
        resize(capacity / RESIZE_FACTOR);
    }
...
template <typename T>
template <typename K>
bool List<T>::removeFirstWhere(bool (*predicate)(const T &check, const K &valueCheck), K valueCheck)
{
    for (size_t i = 0; i < count; i++)
    {
